						...
						<table class="table ">
							...
							<tbody>
								...
								<tr>
									...
									<td scope="row" data-placement="top"><input type="date" name="purchase_date" id="purchase_date"  class="form-control" value="{{ date("Y-m-d") }}"></td>
								</tr>
								<tr>
										...
										<select id="client_id" name="client_id" class="form-control select2">
											...
											@if($client)
												@foreach($client as $row_client)
													<option value='{{$row_client->id}}' @if($client_id != '' && $client_id == $row_client->id) selected='selected' @endif>{{$row_client->company_name}}</option>
												@endforeach
											@endif
										</select>
									</td>
								...
								<tr>
									...
									<td scope="row" data-placement="top">
										<select id="order_type_id" name="order_type_id" class="form-control select2" onchange="changeOrderType(this)">
											@if($ots)
												@foreach($ots as $ot)
													<option value='{{$ot->id}}' <?php if (strtolower($ot->name) === 'manual') echo 'selected' ?>>{{$ot->name}}</option>
												@endforeach
											@endif
										</select>
									</td>
...
<script>
	...
	$("#save_manual_order").validate({
        submitHandler(form){
            ...
            $.ajax({
                type: "POST",
                url: '{{route('orders.store',$client_id)}}',
                data: form1,
                processData: false,
                contentType: false,
