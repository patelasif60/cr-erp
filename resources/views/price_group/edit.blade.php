...
@section('main-content')
    ...
    {{-- @dd($result['group_formulas']->toArray()) --}}
    <div class="row mb-4">
        <div class="col-md-12 mb-4">
            <div class="card text-left">
                 <form action="javascrpt:void(0)" id="edit_price_group">
                    <div class="card-header bg-transparent">
                        <h6 class="card-title task-title">Edit Price Group</h6>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="group_name" class="ul-form__label">Price Group Name:<?php echo $required_span; ?></label>
                                <input type="text" required class="form-control" id="group_name" name="group_name" placeholder="Enter Group Name" value="{{$result->group_name}}">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="store_automator_id" class="ul-form__label">Store Automator ID:<?php echo $required_span; ?></label>
                                <input type="text" required class="form-control" id="store_automator_id" name="store_automator_id" placeholder="Enter Id" value="{{$result->store_automator_id}}">
                            </div>
                            <div class="col-md-3">
                                    ...
                                    <select name="carrier_id" id="carrier_id" class="form-control" required>
                                        ...
                                        @if($carriers)
                                            @foreach ($carriers as $id => $val)
                                                <option value="{{ $id }}" @if($result->carrier_id == $id) selected @endif>{{ $val }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <textarea required name="description" id="description" cols="10" rows="3" class="form-control" placeholder="Enter Description">{{$result->description}}</textarea>
                            </div>
                             <div class="form-group col-md-6 mb-4">
                                ...
                                <select class="form-control select2" id="group_type" name="group_type">
                                    ...
                                    <option value="3PL" {{$result->group_type == "3PL" ? "selected" : " "}}>3PL</option>
                                    <option value="Wholesale" {{$result->group_type == "Wholesale" ? "selected" : " "}}>Wholesale</option>
                                    <option value="In house" {{$result->group_type == "In house" ? "selected" : " "}}>In house</option>
                                    <option value="dropship"{{$result->group_type == "dropship" ? "selected" : " "}}>dropship</option>
                                </select>
                            </div>

                            @if($result['group_formulas'])
                            <h6 class="card-title col-md-12 mt-4 task-title">Package and Material Formula</h6>
                            <div class="form-group @if($result['group_formulas'][0]['group_formula'] == 'Exact') col-md-6 @else col-md-4 @endif js-package_and_matirial">
                                <label for="group_formula" class="">Price Calculation & Formula For Package and material:<?php echo $required_span; ?></label>
                                 <select  onchange="showMultiply()" class="form-control select2" id="group_formula_package_and_matirial" name="group_formula[package_and_matirial]">
                                    <option value="Exact" @if($result['group_formulas'][0]['group_formula'] == 'Exact') selected @endif >Exact</option>
                                    <option value="Multiply" @if($result['group_formulas'][0]['group_formula'] != 'Exact') selected @endif>Multiply</option>
                                </select>
                            </div>
                            <div class="form-group col-md-2 js-parcentage_package_and_matirial @if($result['group_formulas'][0]['group_formula'] == "Exact") d-none @endif">
                                <label for="group_formula" class="">Multiply Value:</label>
                                <input type="text" class="form-control" id="parcentage" name="parcentage[package_and_matirial]" placeholder="Enter Value" @if( $result['group_formulas'][0]['group_formula'] != 'Exact')  value="{{ $result['group_formulas'][0]['group_formula'] }} @endif">
                            </div>
                            <div class="form-group @if($result['group_formulas'][1]['group_formula'] == 'Exact') col-md-6 @else col-md-4 @endif js-coolant_cost">
                                <label for="group_formula" class="">Price Calculation & Formula For Coolant Cost:<?php echo $required_span; ?></label>
                                 <select  onchange="showMultiply()" class="form-control select2" id="group_formula_coolant_cost" name="group_formula[coolant_cost]">
                                    <option value="Exact" @if($result['group_formulas'][1]['group_formula'] == 'Exact') selected @endif >Exact</option>
                                    <option value="Multiply" @if($result['group_formulas'][1]['group_formula'] != 'Exact') selected @endif>Multiply</option>
                                </select>
                            </div>
                            <div class="form-group col-md-2 js-parcentage_coolant_cost @if($result['group_formulas'][1]['group_formula'] == "Exact") d-none @endif">
                                <label for="group_formula" class="">Multiply Value:</label>
                                <input type="text" class="form-control" id="parcentage" name="parcentage[coolant_cost]" placeholder="Enter Value" @if( $result['group_formulas'][1]['group_formula'] != 'Exact')  value="{{ $result['group_formulas'][1]['group_formula'] }} @endif"">
                            </div>

                            <h6 class="card-title col-md-12 mt-4 task-title">Shipping Formula</h6>
                            <div class="form-group @if($result['group_formulas'][2]['group_formula'] == 'Exact') col-md-6 @else col-md-4 @endif js-residential_surcharge">
                                <label for="group_formula" class="">Price Calculation & Formula For Residential Surcharge:<?php echo $required_span; ?></label>
                                 <select  onchange="showMultiply()" class="form-control select2" id="group_formula_residential_surcharge" name="group_formula[residential_surcharge]">
                                    <option value="Exact" @if($result['group_formulas'][2]['group_formula'] == 'Exact') selected @endif >Exact</option>
                                    <option value="Multiply" @if($result['group_formulas'][2]['group_formula'] != 'Exact') selected @endif>Multiply</option>
                                </select>
                            </div>
                            <div class="form-group col-md-2 js-parcentage_residential_surcharge @if($result['group_formulas'][2]['group_formula'] == "Exact") d-none @endif">
                                <label for="group_formula" class="">Multiply Value:</label>
                                <input type="text" class="form-control" id="parcentage" name="parcentage[residential_surcharge]" placeholder="Enter Value" @if( $result['group_formulas'][2]['group_formula'] != 'Exact')  value="{{ $result['group_formulas'][2]['group_formula'] }} @endif">
                            </div>

                            <div class="form-group @if($result['group_formulas'][3]['group_formula'] == 'Exact') col-md-6 @else col-md-4 @endif js-remote_area_surcharge">
                                <label for="group_formula" class="">Price Calculation & Formula For Remote Area Surcharge:<?php echo $required_span; ?></label>
                                 <select  onchange="showMultiply()" class="form-control select2" id="group_formula_remote_area_surcharge" name="group_formula[remote_area_surcharge]">
                                    <option value="Exact" @if($result['group_formulas'][3]['group_formula'] == 'Exact') selected @endif >Exact</option>
                                    <option value="Multiply" @if($result['group_formulas'][3]['group_formula'] != 'Exact') selected @endif>Multiply</option>
                                </select>
                            </div>
                            <div class="form-group col-md-2 js-parcentage_remote_area_surcharge @if($result['group_formulas'][3]['group_formula'] == "Exact") d-none @endif">
                                <label for="group_formula" class="">Multiply Value:</label>
                                <input type="text" class="form-control" id="parcentage" name="parcentage[remote_area_surcharge]" placeholder="Enter Value" @if( $result['group_formulas'][3]['group_formula'] != 'Exact')  value="{{ $result['group_formulas'][3]['group_formula'] }} @endif">
                            </div>

                            <div class="form-group @if($result['group_formulas'][4]['group_formula'] == 'Exact') col-md-6 @else col-md-4 @endif js-delivary_area_surcharge">
                                <label for="group_formula" class="">Price Calculation & Formula for Delivary Area Surcharge:<?php echo $required_span; ?></label>
                                 <select  onchange="showMultiply()" class="form-control select2" id="group_formula_delivary_area_surcharge" name="group_formula[delivary_area_surcharge]">
                                    <option value="Exact" @if($result['group_formulas'][4]['group_formula'] == 'Exact') selected @endif >Exact</option>
                                    <option value="Multiply" @if($result['group_formulas'][4]['group_formula'] != 'Exact') selected @endif>Multiply</option>
                                </select>
                            </div>
                            <div class="form-group col-md-2 js-parcentage_delivary_area_surcharge @if($result['group_formulas'][4]['group_formula'] == "Exact") d-none @endif">
                                <label for="group_formula" class="">Multiply Value:</label>
                                <input type="text" class="form-control" id="parcentage" name="parcentage[delivary_area_surcharge]" placeholder="Enter Value" @if( $result['group_formulas'][4]['group_formula'] != 'Exact')  value="{{ $result['group_formulas'][4]['group_formula'] }} @endif">
                            </div>


                            <div class="form-group @if($result['group_formulas'][5]['group_formula'] == 'Exact') col-md-6 @else col-md-4 @endif js-extended_delivary_area_surcharge">
                                <label for="group_formula" class="">Price Calculation & Formula for Extended Delivary Area Surcharge:<?php echo $required_span; ?></label>
                                 <select  onchange="showMultiply()" class="form-control select2" id="group_formula_extended_delivary_area_surcharge" name="group_formula[extended_delivary_area_surcharge]">
                                    <option value="Exact" @if($result['group_formulas'][5]['group_formula'] == 'Exact') selected @endif >Exact</option>
                                    <option value="Multiply" @if($result['group_formulas'][5]['group_formula'] != 'Exact') selected @endif>Multiply</option>
                                </select>
                            </div>
                            <div class="form-group col-md-2 js-parcentage_extended_delivary_area_surcharge @if($result['group_formulas'][5]['group_formula'] == "Exact") d-none @endif">
                                <label for="group_formula" class="">Multiply Value:</label>
                                <input type="text" class="form-control" id="parcentage" name="parcentage[extended_delivary_area_surcharge]" placeholder="Enter Value" @if( $result['group_formulas'][5]['group_formula'] != 'Exact')  value="{{ $result['group_formulas'][5]['group_formula'] }} @endif">
                            </div>
                            @endif
                             <div class="form-group col-md-6">
                                <label for="weight_multiplier" class="ul-form__label">Weight Multiplier:</label>
                                <input type="number" class="form-control" id="weight_multiplier" name="weight_multiplier" value="{{$result->weight_multiplier}}" placeholder="Enter Value">
                           </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="number" class="form-control" id="credit_card_fees" name="credit_card_fees" value="{{$result->credit_card_fees}}" placeholder="Enter Value">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="marketplace_fees" class="ul-form__label">Marketplace Fees:</label>
                                <input type="number" class="form-control" id="marketplace_fees" name="marketplace_fees" value="{{$result->marketplace_fees}}" placeholder="Enter Value">
                            </div>
                           <h6 class="card-title col-md-12 mt-4 task-title">PriceGroup Formula</h6>
                            <div class="form-group col-md-6">
                                ...
                                <input type="number" class="form-control" id="markup_price_group" name="markup_price_group" placeholder="Enter Value" value="{{$result->markup_price_group}}">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="markup_total_cost" class="ul-form__label">% Markup: Total Cost:</label>
                                <input type="number" class="form-control" id="markup_total_cost" name="markup_total_cost" value="{{$result->markup_total_cost}}" placeholder="Enter Value">
                           </div>
                            <div class="form-group col-md-6">
                                <label for="markup_product_materials_cost" class="ul-form__label">% Markup: Product & Materials Cost:</label>
                                <input type="number" class="form-control" id="markup_product_materials_cost" name="markup_product_materials_cost" placeholder="Enter Value" value="{{$result->markup_product_materials_cost}}">
                            </div>
                            ...
                            <div class="row">
                                <div class="row form-group col-md-6">
                                    ...
                                    <div class="form-group col-md-6">
                                        <input type="hidden" name="lobs" id="lobs" value="{{$result->lobs}}">
                                ...
                                <div class="form-group col-md-6">
								    <input type="hidden" name="chanel_ids" id="chanel_ids" value="{{ $result->chanel_ids }}">
								    <div class="row"  id="client_channel_container"></div>
							    </div>
                            </div>
                            <!-- <div class="form-group col-md-5 ml-16">
                                <div class="row ">
                                        ...
                                        <div class="col-md-6 mb-4">
                                            <div class="card mb-4" style="max-height: 376px;overflow-y: auto;">
                                                ...
                                                <div class="card-body col-md-12"  ondrop="drop(event)" ondragover="allowDrop(event)" id="mstr{{$val->id}}">
                                                    @foreach($val->subCost()->whereNotIn('id',json_decode($result->priceGroupCostBlock->sub_cost_ids))->get() as $subKey => $subVal )
                            ...
                            <input type="hidden" readonly name="sub_cost_id" id="sub_cost_id" value="{{implode(',',json_decode($result->priceGroupCostBlock->sub_cost_ids))}}">
                            <!-- <div class="col-md-1"></div>
                            <div class="col-md-5 border border-success" ondrop="drop(event)" ondragover="allowDrop(event)" id="drag4">
                                @foreach($subCostData as $subKey => $subVal )
                                    <div style="height: 50px" draggable="true" class="border border-info mt-2 mb-4 {{str_replace(' ','_',$subVal->masterCost->name)}} ml-16" data-id="{{$subVal->id}}" ondragstart="drag(event)" data-type="sub" id="sub{{$subVal->id}}">
                                        {{$subVal->cost_name}}
                                    </div>
                                @endforeach
                            </div> -->
                                        ...
                                        <div class="card mb-4" style="max-height: 337px;overflow-y: auto;">
                                            ...
                                            <div class="drop mastercost{{$val->id}}">
                                                @foreach($val->subCost as $subKey => $subVal )
                                                    @if(!in_array($subVal->id,json_decode($result->priceGroupCostBlock->sub_cost_ids)))
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="js-drop-assigned border">
                                    @foreach($masterCost as $key=>$val)
                                        @foreach($val->subCost as $subKey => $subVal )
                                            @if(in_array($subVal->id,json_decode($result->priceGroupCostBlock->sub_cost_ids)))
                                                <div style="height: 50px" class="js-cost-click border border-info mt-2 mb-4 {{str_replace(' ','_',$val->name)}} ml-16" data-msterId="{{$val->id}}" data-id="{{$subVal->id}}" id="sub{{$subVal->id}}">
                                                    {{$subVal->cost_name}}
                                                </div>
                                            @endif
                                        @endforeach
...
<script type="text/javascript">
     $(document).ready(function () {
        var sub_cost_id = $("#sub_cost_id").val();
        sub_cost_array = sub_cost_id.split(',');
        for (let i of sub_cost_array) {
         changeGrid(i,'add') // logs 3, 5, 7
        }
    });
    ...
    // showMultiply = () => {
    //     if($('#group_formula').val() == 'Multiply'){
    //         $('.js-formula').removeClass('col-md-6');
    //         $('.js-formula').addClass('col-md-3');
    //         $('.js-parcentage').removeClass('d-none');
    //     }
    //     else{
    //         $('.js-formula').removeClass('col-md-3');
    //         $('.js-formula').addClass('col-md-6');
    //         $('.js-parcentage').addClass('d-none');
    //     }
    ...
    showMultiply = () => {
     ...
     $("#edit_price_group").validate({
        submitHandler(form){
            $(".submit").attr("disabled", true);
            $('div#preloader').show();
            var form_cust = $('#edit_price_group')[0];
            let form1 = new FormData(form_cust);
            $.ajax({
                type: "POST",
                url: '{{route('pricegroup.update',$result->id)}}',
                data: form1,
                processData: false,
                contentType: false,
