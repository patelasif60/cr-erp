...
<div class="modal-dialog mt-5 modal-lg" style="min-width: 70%">
    <div class="modal-content">
      <div class="modal-header bg-light">
        <h5 class="modal-title" id="createEventModalLabel">Order By Transit Days</h5>
      ...
      <div class="modal-body">
              ...
              <tbody>
                @if(isset($temp_count) && count($temp_count) > 0)
                  @foreach($temp_count as $key => $value)
                    <tr style="font-weight: bold">                          
                        ...
                        @foreach (range(-1, 5) as $day)  
                          @if (isset($value[$day]) && $value[$day] !== '')
                            <td>
                              <a href="javascript:showDetailsTdGr('{{ $value[$day] }}');">
                        ...
                        @if (count(array_unique($total)) > 0)
                          <td>
                            <a href="javascript:showDetailsTdGr('{{ implode(',', array_unique($total)) }}');">
                    ...
                    @if(isset($type_transit_count[$key]))
                      @foreach ($type_transit_count[$key] as $val_key => $val_value)
                        <tr>
                          <td align="left" class="pl-40">{{ $val_key }}</td>
                          @foreach (range(-1, 5) as $day)
                            @if (isset($val_value[$day][1]) && $val_value[$day][1] !== '')
                              <td>
                                <a href="javascript:showDetailsTdGr('{{ $val_value[$day][1] }}');">
                                  {{ $val_value[$day][0] }}
                                </a>
                              </td>
                            @else
                          ...
                          @if (count(array_unique($val_total)) > 0)
                            <td>
                              <a href="javascript:showDetailsTdGr('{{ implode(',', array_unique($val_total)) }}');">
        ...
        <div id="item_table" class="table-responsive" style="display: none">
          <div class="modal-header bg-light">
          ...
          <table id="datatableOrderTd" class="table table-bordered text-center mt-6">
            ...
            <tbody id="datatableOrderTdBody">              
            </tbody>
          </table>
        </div>
        <button data-dismiss="modal" type="button" class="btn btn-danger float-right">Close</button>            
        <button id="show_tb_btn" type="button" class="btn btn-success float-right mr-2" onclick="hideTable()" style="display: none">Hide Details</button>            
      </div>
    </div>
</div>
<script>
  function showDetailsTdGr(subOrderNumbers) {
    // Remove Old Rows
    removeRows();
  ...
  function appendRows(data) {
    ...
    let tableRef = document.getElementById('datatableOrderTdBody');
    let tableRefDiv = document.getElementById('item_table');
    ...
    document.getElementById("show_tb_btn").style.display = "";
  }

  function hideTable() {
    removeRows();
    document.getElementById("item_table").style.display = "none";
    document.getElementById("show_tb_btn").style.display = "none";
  }
