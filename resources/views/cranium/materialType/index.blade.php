        ...
        <div class="col-md-12 mb-4">
            <div class="card text-left">
                <div class="card-header text-right bg-transparent">
                    ...
                    <a href="javascript:void(0);" class="btn btn-primary btn-md m-1" onClick="openModal()"><i class="i-Add text-white mr-2"></i>Add Material Type</a>
                    @endif
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="material_type_datatable" class="display table table-striped table-bordered" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Material Type</th>
                                    <th>Action</th>
            ...
            <div class="modal-content">
                ...
                <form  method="POST" data-form="add" action="javascript:void(0)" id="add_form" >
                    ...
                    <div class="modal-body">
                        <lable for="material_type">Material Type</lable>
                        <input type="text" class="form-control" name="material_type" placesholder="Add new material type" id="material_type" style="width:100%;"/> 
                    </div> 
...
@section('page-js')
    ...
    <script>
       $(document).ready(function () {
            ...
            var table_html_td = '';
            var i = 1;
            var dt = $('#material_type_datatable').DataTable({
                destroy: true,
                processing: true,
                serverSide: true,
                responsive: true,
                ...
                ajax: '{!! route('datatable.materialtypelist') !!}',
                columns: [
        ...
        function openModal(){
            ...
            $('#hmodelHeader').text('Add Material Type');
        ...
        function openEditModal(id,type){
            ...
            $('#material_type_id').val(id);
            $('#MyModal').modal('show');
            $('#add_form').attr('data-form','edit');
            $('#hmodelHeader').text('Edit Material Type');
            $('.btn-txt').text('Save');
        }
        ...
        $("#add_form").validate({
            submitHandler(form){
                ...
                var url = '{{ route('materialtype.store') }}';
                if(formtype == 'edit'){
                    url = '{{ route('materialtype.update') }}'
                }
                $.ajax({
                    type: "POST",
                    url: url,
        ...
        deleteMaterialType = (id) =>{
            swal({
                    ...
                    if(result)
                    {
                        $.ajax({
                            type: "Delete",
                            url: '{{ route('materialtype.destroy') }}',
                            data: {'id':id},
                            success: function( response ){
                                $("#preloader").hide();
                                location.reload()
