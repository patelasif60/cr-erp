    ...
    <div class="modal-content">
        ...
        <form  method="POST" action="javascript:void(0)" id="edit_manufacturer_form" >
            ...
            <label for="manufacturer_id">Manufacturer Name</label>
                <input type="text" class="form-control" name="manufacturer_name" placesholder="Edit manufacturer" id="manufacturer_name" style="width:100%;" value="{{ $row['manufacturer_name'] }}"/> 
            </div> 
...
$("#edit_manufacturer_form").validate({
    submitHandler(form){
        ...
        $.ajax({
            type: "POST",
            url: '{{route('manufacturer.update',$row['id'])}}',
            data: form1, 
            processData: false,
            contentType: false,
