        ...
        <div class="col-md-12 mb-4">
            <div class="card text-left">
                <div class="card-header text-right bg-transparent">
                    ...
                    <a href="javascript:void(0);" class="btn btn-primary btn-md m-1" onClick="openModal()"><i class="i-Add text-white mr-2"></i>AddBilling Notes</a>
                    @endif
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="billing_note_datatable" class="display table table-striped table-bordered" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Option</th>
                                    <th>Action</th>
            ...
            <div class="modal-content">
                ...
                <form  method="POST" data-form="add" action="javascript:void(0)" id="add_form" >
                    ...
                    <div class="modal-body">
                        <lable for="option">Option</lable>
                        <input type="text" class="form-control" name="option" placesholder="Add new option" id="option" style="width:100%;"/> 
                    </div> 
...
@section('page-js')
    ...
    <script>
       $(document).ready(function () {
            ...
            var table_html_td = '';
            var i = 1;
            var dt = $('#billing_note_datatable').DataTable({
                destroy: true,
                processing: true,
                serverSide: true,
                responsive: true,
                ...
                ajax: '{!! route('datatable.billingnotesList') !!}',
                columns: [
        ...
        function openModal(){
            ...
            $('#hmodelHeader').text('Add Billing Note');
            $('#id').val('');
            $('.btn-txt').text('Add');
        ...
        function openEditModal(id,type){
            ...
            $('#hmodelHeader').text('Edit Billing Note');
            $('.btn-txt').text('Save');
        }
        ...
        $("#add_form").validate({
            submitHandler(form){
                ...
                var url = '{{ route('billingnotes.store') }}';
                if(formtype == 'edit'){
                    url = '{{ route('billingnotes.update') }}'
                }
                $.ajax({
                    type: "POST",
                    url: url,
        ...
        deleteBillingNote = (id) =>{
            swal({
                    ...
                    if(result)
                    {
                        $.ajax({
                            type: "Delete",
                            url: '{{ route('billingnotes.destroy') }}',
                            data: {'id':id},
                            success: function( response ){
                                $("#preloader").hide();
                                location.reload()
