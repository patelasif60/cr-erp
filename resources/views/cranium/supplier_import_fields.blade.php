<!-- <div class="modal-dialog mt-5">
    ...
    <form id="save_mapping" class="form-horizontal" method="POST" action="{{ route('suppliers.saveSupplierImportHeaders') }}">
                {{ csrf_field() }}
               <input type="hidden" name="supplier_id" value="{{$supplier_id}}">
               <input type="hidden" name="csv_format" value="{{$csv_format}}">
                <table class="col-md-11 table-striped text-center">
                    ...
                    <tbody>
                        ...
                        @if($csv_format == 'packaging_materials')
                            @foreach (config('mapping.'.$csv_format) as $db_field => $desc)
                                <tr>
                                    <td>{{ $desc }}</td>
                                    <td>
...
$("#save_mapping").validate({
	submitHandler(form){
		...
		$.ajax({
			type: "POST",
			url: '{{ route('suppliers.saveSupplierImportHeaders') }}',
			data: form1,
			processData: false,
			contentType: false,
