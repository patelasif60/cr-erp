			...
			<div class="card-body">
				...
				<div class="mb-3 row" style="">
					<div class="col-md-9">
						...
						<a href="{{ url('/allmasterproductlsts2') }}" class="btn btn-warning">Clear Filters</a>
						@endif
					</div>
					<div class="col-md-3">
						@if($id != NULL)
				...
				<!-- <div class="row">
					<div class="col-lg-12">
						<button class="btn btn-primary btn-block btn-lg" type="button" onClick="GetActiveProducts()">Search</button>
					</div>
				</div> -->
			</div>
		</div>
	...
	<div class="col-md-12">
		<div class="card o-hidden mb-4">
			<div class="card-header">
				<div class="row">
					<div class="col-md-4">
						<h3 class="w-50 float-left card-title m-0">Active Product Listings 2</h3>
					</div>
					<!-- <div class="col-md-2">
					</div> -->
					<div class="col-md-2">
						...
						@if($product_listing_filter)
							@foreach($product_listing_filter as $key => $row_product_listing_filter)
								...
								<div class="dropdown mb-2 fl_{{ $row_product_listing_filter->sorting_order }}" style="<?php if($display == true) echo "display:inline-block !important"; else echo "display:none"?>">
									<a class="btn btn-secondary dropdown-toggle dropdown-filter" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" >
			...
			<ul class="" id="#smart_filters"  style="list-style-type:none;">
				@if($smart_filters)
					@foreach($smart_filters as $smart_fil)
						<li class="m-2"><a href="{{url('/allmasterproductlsts2')}}/{{$smart_fil->id}}" class="font-weight-bold ml-2" style="<?php if($id != NULL && $smart_fil->id == $id) echo 'color: #19bef4';?>">{{ $smart_fil->filter_name}}</a></li>
					@endforeach
				@endif
			</ul>
      </div>
		...
		table1 = $('#datatable').DataTable({
			...
			buttons: [
				{
					...
					title: 'Cranium_<?php echo date("Ymd");?>',
            ...
            columns: [
				<?php
					if($product_listing_filter){
						foreach($product_listing_filter as  $key => $row_product_listing_filter){
                            echo "{data: '".$row_product_listing_filter->column_name."', name: '".$row_product_listing_filter->column_name."',searchable: false},";
						}
					}
				?>
	...
	function ExportExcel(v){
		...
		$.ajax({
            ...
            success: function( response,status, xhr ) {
                ...
                var link = document.createElement('a');
                link.href = window.URL.createObjectURL(blob);
                link.download = "Cranium_"+"{{date('Ymd')}}"+".xlsx";
