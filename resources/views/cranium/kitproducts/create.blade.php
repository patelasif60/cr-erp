...
<!--begin::form-->
	<form method="POST" action="{{ route('kits.store') }}" enctype="multipart/form-data" id="product_add">
	@csrf
	...
	var table = $('#parent_products').DataTable({
        ...
        ajax: {
			...
			data: function(d){
				...
				d.client_id = @if(isset($req['client_id'])) '{{$req['client_id']}}' @else '' @endif
			}
		},
        columns: [
            {data: 'ETIN', name: 'ETIN'},
...
$("#add_new_product_type_request_form").validate({
    submitHandler(form){
        ...
        $.ajax({
            type: "POST",
            url: '{{route('new_product_type_kit_request')}}',
            data: form1,
            processData: false,
            contentType: false,
            success: function( response ) {
