...
<!--begin::form-->
	<form method="POST" action="{{ route('addmasterproduct') }}" enctype="multipart/form-data" id="product_add">
	@csrf
	@method('put')
						...
						<select id="status" name="status" class="form-control select2" >
							...
							@if($product_status)
								@foreach($product_status as $row_status)
									<option value="{{$row_status->product_status}}">{{$row_status->product_status}}</option>
								@endforeach
							@endif
						</select>
					</div>
									...
									<table class="table card_">
										<tbody>
											<tr>
												...
												<td>
													<select id="current_supplier" name="current_supplier" class="form-control select2" @if(request()->supplier) disabled @endif >
															<option value=''> -- Select a value  -- </option>
														@if ($supplier)
															@foreach($supplier as $key => $value)
																<option value="{{ $key }}" @if(strtoupper(request()->supplier) == strtoupper($value)) selected @endif>{{ $value }}</option>
															@endforeach
														@endif
													</select>
												</td>
									...
									<select class="form-control image-type select2" name="image[1][image_type]">
										...
										@if($image_types)
											@foreach($image_types as $image_type)
												<option value="{{$image_type->id}}">{{$image_type->image_type}}</option>
											@endforeach
										@endif
									</select>
								</div>
...
$('#brand').change(function () {
	 var id = $(this).val();
	 var myurl1 = "{{url('getmanufacturer')}}" +"/"+ id;
