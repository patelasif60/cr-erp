        ...
        <div class="modal-body">
            <table class="table table-bordered text-center" id="order_details">
                ...
                <tbody>
                            ...
                            <select id="box_name" class="form-control select2" name="box_name" required>
                                ...
                                @if (isset($outer_box))
                                    @foreach ($outer_box as $ob)                                        
                                        <option value="{{ $ob->id }}" <?php if($ob->id == $ccob->box_id) echo 'selected';?>>
                                            {{ $ob->product_description }} - {{ $ob->ETIN }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                            ...
                            <select required id="client_name" class="form-control select2" name="client_name" onchange="fetchChannelAndProduct(this.value);">
                                ...
                                @if (isset($clients))
                                    @foreach ($clients as $cl)                                        
                                        <option value="{{ $cl->id }}" <?php if($cl->id == $ccob->client_id) echo 'selected';?>>
                                            {{ $cl->company_name }}
                                        </option>
                                    @endforeach
                                @endif
                    ...
                    <tr>
                        ...
                        <td>
                            <select id="channel_name" class="form-control select2" name="channel_name[]" multiple>
                                <option value="">-Select-</option>
                                @isset($channels)
                                    <?php $channel_list = explode(',', $ccob->channel_ids); ?>                              
                    ...
                    <tr>
                        ...
                        <td>
                            <select required id="product_name" class="form-control select2" name="product_name[]" multiple>
                                <option value="">-Select-</option>
                                @isset($products)
                                    <?php $product_list = explode(',', $ccob->product_ids); ?>                              
                    ...
                    <tr>
                        <td>Transit Day:</td>
                        <td><input required type="number" id="transit_day" name="transit_day" class="form-control" value="{{ $ccob->transit_days }}"/></td>                        
                    </tr>
                    <tr>
                        <td>Max Item Count:</td>
                        <td><input required type="number" id="max_item_count" name="max_item_count" class="form-control" value="{{ $ccob->max_item_count }}" /></td>                        
                    </tr>
                </tbody>
            </table>
            <button type="button" class="btn btn-success float-right" onclick="storeCustomBox({{ $ccob->id }});">Save</button>
        </div>
    </div>
</div>
