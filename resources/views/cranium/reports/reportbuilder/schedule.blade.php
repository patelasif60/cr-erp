			...
			<div class="card-body">
				...
				<div class="mb-3 row" style="">
                    <div class="col-md-3 mb-4">
                        ...
                        <input type="email" data-parsley-excluded="true" placeholder="multiple email ids with comma separated" id="emails" name="emails" class="form-control" multiple>   
                    </div>
					<div class="col-md-3 mb-4">
                    ...
                    <div class="col-md-3 mb-4">
                        <label for="schedule_type" class="ul-form__label">Schedule Type:</label>
                        <select required data-parsley-errors-container="#errorschedule_type" class="form-control select2" onchange="scheduleType(this)" id="schedule_type" name="schedule_type">
                            <option value="">--Select--</option>
                            <option value="daily">Daily Report</option>
                            <option value ="weekly">Weekly Report</option>
                            <option value="monthly">Monthly Report</option>
                        </select>
                        <div id="errorschedule_type" class="text-danger"></div>
                    </div>
                     <div class="col-md-3 mb-4">
                        <label for="schedule_value" class="ul-form__label">Schedule day:</label>
                        <select required data-parsley-errors-container="#errorschedule_value" class="form-control select2" id="schedule_value" name="schedule_value">
                            <option value="">--Select--</option>
                            <option  ref_val="daily" value="daily">Daily</option>
                            <option  ref_val="weekly" value ="Monday">Monday</option>
                            <option  ref_val="weekly" value ="Tuesday">Tuesday</option>
                            <option  ref_val="weekly" value ="Wednesday">Wednesday</option>
                            <option  ref_val="weekly" value ="Thursday">Thursday</option>
                            <option  ref_val="weekly" value ="Friday">Friday</option>
                            <option  ref_val="weekly" value ="Saturday">Saturday</option>
                            <option  ref_val="weekly" value ="Sunday">Sunday</option>
                            @for($i=1;$i<=31;$i++)
                                <option  ref_val="monthly" value="{{$i}}">{{$i}}</option>
                            @endfor
                        </select>
                        <div id="errorschedule_value" class="text-danger"></div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-3" style="margin-top: 2.5rem!important;">
                        <button type="button" onClick="buildReport()" class="btn btn-primary submit">Save Schedule</button>
                    </div>
                </div>
                </form>
			</div>
...
$secheduleOptions = $('#schedule_value').find( 'option' );

function scheduleType(obj){
    if(obj.value != ''){
        $("#schedule_value").html( $secheduleOptions.filter( '[ref_val="' + obj.value + '"]' ) );
    }else{
        $("#schedule_value").html( $secheduleOptions.filter( '[ref_val=""]' ) );
    }
}
...
function buildReport(){  
   ...
   // if ($('#report').parsley().validate() && $('#report_type').parsley().validate() && $('#client_id').parsley().validate() ) {    
        var report = $("#report").val();
        var reportSubType = $("#report_type").val();
        $(".submit").attr("disabled", true);
        var url = '{{ route('report-schedule-submit') }}';
        ...
        if($("#report_type").val()== "own") {
            ...
            form1.append("emails",$("#emails").val())
            form1.append("schedule_value",$("#schedule_value").val())
            form1.append("schedule_type",$("#schedule_type").val())
        }
        else{    
