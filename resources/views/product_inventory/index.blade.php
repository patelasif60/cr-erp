        ...
        <div class="card text-left">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <a href="{{route('productinventory.exportInv')}}" class="btn btn-primary">Export CSV</a>
                </div>
            </div>
        </div>
    </div>
                            ...
                            <table class="table">
                                @if ($warehouses)
                                    @foreach($warehouses as $key=>$warehouse)
                                        <tr>
                                            <td>{{ $warehouse }}</td>
                                            <td><input type="checkbox" name="warehouses[]" value="{{$key}}"></td>
                                        </tr>
                                    @endforeach
                                @endif
                            </table>
...
<script>
   $(document).ready(function () {
        ...
        var dt = $('#inventory_table').DataTable({
            ...
            ajax: '{!! route('productinventory.productinventorylist') !!}',
            columns: [
                { data: 'ETIN', name: 'ETIN' },
   ...
   function openModal(){
    ...
    function openEditModal(id){
        var url = '{{ route('productinventory.edit') }}';
        $.ajax({
            type: "POST",
            url: url,
            data: {'id':id}, 
    ...
    $('body').on('click', '.edit-form', function() {
        url = '{{ route('productinventory.update') }}';
    ...
    $("#add_form").validate({
        submitHandler(form){
            ...
            let form1 = new FormData(form_cust);
            var url = '{{ route('productinventory.store') }}';
            if(formtype == 'edit'){
                url = '{{ route('productinventory.update') }}'
            }
            $.ajax({
