<div class="modal-dialog mt-5">
    <div class="modal-content">
      <div class="modal-header bg-light">
        <h5 class="modal-title" id="editEventModalLabel">Edit Details</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
          <form action="javascript:void(0);" method="POST" id="editEventForm">
            {{ csrf_field() }}
                    ...
                    <select name="owner" id="owner" class="form-control">
                        ...
                        @if($users)
                            @foreach ($users as $id => $val)
                                <option value="{{ $id }}" @if($row->owner == $id) selected @endif>{{ $val }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
...
<script>
	...
	submitHandler(form){
		...
		$.ajax({
			type: "POST",
			url: '{{ route('clients.updateWarehouseAndFulfillment',$row->id) }}',
			data: form1,
			processData: false,
			contentType: false,
