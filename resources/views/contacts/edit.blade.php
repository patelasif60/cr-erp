    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          <form action="javascript:void(0);" method="POST" id="editContactForm">
            ...
            <div class="row">
                <div class="col-md-8">
                    <label for="is_contract">Cranium Account</label>
                    <select name="is_contract" id="is_contract" class="form-control">
                        <option value="1" @if($row->is_contract == 1) selected @endif>Yes</option>
                        <option value="0" @if($row->is_contract == 0) selected @endif>No</option>
                    </select>
                </div>
            </div>
            <div class="row">
                <div class="col-md-8">
                    <label for="contact_note">Contact Notes </label>
...
<script>
	...
	submitHandler(form){
		...
		$.ajax({
			type: "POST",
			url: '{{ route('clients.updateContact',$id) }}',
			data: form1,
			processData: false,
			contentType: false,
