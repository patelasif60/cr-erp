        ...
        <form  method="POST" action="javascript:void(0)" id="add_carrier_shipping" >
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="service_type_etailer" value="{{ $etailerServices['etailer_service_name'] }}" />
                </div>
                <div class="form-group">
                    ...
                    <select id="service_type_ups" name="service_type_ups" class="form-control select2" >
                        @foreach($shippingServiceTypes as $shippingServiceType)
                            @if($shippingServiceType['service_type'] == 'ups')
                                <option  {{$etailerServices['ups_service_type_id'] == $shippingServiceType['id'] ? 'selected' : ''}} value="{{$shippingServiceType['id']}}">{{$shippingServiceType['service_name']}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                    ...
                    <select id="service_type_fedex"  name="service_type_fedex" class="form-control select2" >
                        @foreach($shippingServiceTypes as $shippingServiceType)
                            @if($shippingServiceType['service_type'] == 'fedex')
                                <option  {{$etailerServices['fedex_service_type_id'] == $shippingServiceType['id'] ? 'selected' : ''}} value="{{$shippingServiceType['id']}}">{{$shippingServiceType['service_name']}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
