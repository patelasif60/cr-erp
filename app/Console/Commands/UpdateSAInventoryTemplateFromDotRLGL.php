...
use DB;
use Log;
class UpdateSAInventoryTemplateFromDotRLGL extends Command
{
    /**
    ...
    public function handle()
    {
        ...
        if($get_sku){
            // foreach($get_sku as $row_sku){
                try{
                    UpdateInventoryDotRLGL::dispatch($get_sku);
                    Log::channel('UpdateSAInventoryTemplateFromDotRLGL')->info('Job Created For SKU checking and Inventory Update');
                }catch(\Exception $e){
