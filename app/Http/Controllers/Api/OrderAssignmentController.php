...
class OrderAssignmentController extends Controller 
{
    ...
    public function assignOrdersToPicker(Request $request, $pickerId, $wah) {
        ...
        foreach ($orders as $order) {
            ...
            if($order->fulfilled_by == ''){
                $response = ["error"=>true, 'message' => 'Order fullfilled by is empty of sub order number '.$order->sub_order_number];
                return response($response, 404);  
            }
        ...
        try {            
            $batch = $this->assignOrders($orders, $osge, $maxBatch, $wah, $groups, $day);
    ...
    private function assignOrders($orders, $osge, $maxBatch, $wh, $groups, $day) {
        ...
        foreach ($orders as $order) {
            ...
            Log::channel('OrderAssignmentController')->info('Fulfilled By: '.$order->fulfilled_by );
            Log::channel('OrderAssignmentController')->info('Sub Order Number: '.$order->sub_order_number );

            $fullfilledBy = strtolower($order->fulfilled_by);
            $temp = $this->getTemp($fullfilledBy, $order->sub_order_number);
            

            $packType = $this->getPackagingType([$temp], $groups);
            Log::channel('OrderAssignmentController')->info('Order Id: ' . $order->id . '. Temp: ' . $temp . '. Pack Type: ' . $packType);
            Log::channel('OrderAssignmentController')->info('getPackagingType: '.$packType.' temp: '.json_encode($temp).' groups: '.json_encode($groups));
            if (!isset($packType)) {
                Log::channel('OrderAssignmentController')->info('Picker is not assigned any groups');
                break;
            }
            Log::channel('OrderAssignmentController')->info('Pack Type to Assign: ' . $packType);
            ...
            Log::channel('OrderAssignmentController')->info('strcmp: ' . strcmp($temp, $packType));
            $isTempAndTransitElligible = $this->isTempAndTransitElligible($temp, $transitDay, $osge);
            Log::channel('OrderAssignmentController')->info('isTempAndTransitElligible: ' . $isTempAndTransitElligible);

            Log::channel('OrderAssignmentController')->info('Condition 1: ' . (strcmp($temp, $packType) == 0 ? 'true' : 'false'));
            Log::channel('OrderAssignmentController')->info('Condition 2: ' . ($isTempAndTransitElligible ? 'true' : 'false'));

            if (!(strcmp($temp, $packType) === 0 && $isTempAndTransitElligible)) {
        ...
        Log::channel('OrderAssignmentController')->info('Batch:'. json_encode($batch));
        Log::channel('OrderAssignmentController')->info('orderNumbers:'. json_encode($orderNumbers));
    ...
    private function isTempAndTransitElligible($temp, $transit, $osge) {
        Log::channel('OrderAssignmentController')->info('To check. Temp: ' . $temp . ', Day: ' . $transit.', osge: '.json_encode($osge));
        foreach ($osge as $os) {
